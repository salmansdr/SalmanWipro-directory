      ...
      if (config) {
        ...
        setBhkRoomDetails(prevDetails => {
          ...
          if (Array.isArray(config.rooms)) {
            ...
            if (config.circulation_space_sqft) {
              modalDetails['Count']['Circulation Space'] = 1;
              const side = Math.round(Math.sqrt(config.circulation_space_sqft));
              modalDetails['Length (ft)']['Circulation Space'] = side;
              modalDetails['Width (ft)']['Circulation Space'] = side;
            }
          }
        ...
        setBhkRoomDetails(prevDetails => {
          ...
          if (newRow.type && newRow.area && Array.isArray(allBhkConfigs)) {
            ...
            if (config && Array.isArray(config.rooms)) {
              ...
              // Add Circulation Space if present
              if (config.circulation_space_sqft) {
                modalDetails['Count']['Circulation Space'] = 1;
                const side = Math.round(Math.sqrt(config.circulation_space_sqft));
                modalDetails['Length (ft)']['Circulation Space'] = side;
