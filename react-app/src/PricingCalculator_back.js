...
const PricingCalculator = () => {
  ...
  const [bcFloor, setBCFloor] = useState('Foundation');
  const [bcFloorHeight, setBCFloorHeight] = useState(10);
  const [bcGridSpacing, setBCGridSpacing] = useState(12);
...
const totalCarpetArea = (Number(width) && Number(depth)) ? (Number(width) * Number(depth) * (carpetPercent/100)) : 0;
  ...
  floor: floorIdx === 0 ? 'Foundation' : floorIdx === 1 ? 'Ground Floor' : floorIdx === 2 ? '1st Floor' : floorIdx === 3 ? '2nd Floor' : floorIdx === 4 ? '3rd Floor' : `${floorIdx - 1}th Floor`,
